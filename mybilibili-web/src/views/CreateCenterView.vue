      ...
      <main class="content-area">
        ...
        <div v-else-if="currentActive === 'data'" class="content-section">
          ...
          <div class="content-body">
            ...
            <!-- 核心数据概览 -->
            <div class="data-overview" v-if="activeDataTab === 'overview'" v-loading="dataOverviewLoading">
              <div class="overview-header">
                <h3 class="overview-title">核心数据概览 <el-tag size="small" type="info">更新至{{ dataOverview.updateTime || '今日' }}</el-tag></h3>
              ...
              <div class="core-data">
                ...
                <div class="data-card play-count">
                  ...
                  <div class="data-value">{{ formatNumber(dataOverview.totalViews) }}</div>
                  <div class="data-change" v-if="dataOverview.viewsIncrease > 0">
                    <el-icon><ArrowUp /></el-icon>
                    <span class="increase">+{{ dataOverview.viewsIncrease }}</span>
                  </div>
                </div>
                
                <!-- 粉丝卡片 -->
                <div class="data-card">
                  <div class="data-title">粉丝数</div>
                  <div class="data-value">{{ formatNumber(dataOverview.totalFollowers) }}</div>
                  <div class="data-change" v-if="dataOverview.followersIncrease > 0">
                    <el-icon><ArrowUp /></el-icon>
                    <span class="increase">+{{ dataOverview.followersIncrease }}</span>
                  </div>
                </div>
                
                <!-- 稿件数卡片 -->
                <div class="data-card">
                  <div class="data-title">稿件数</div>
                  <div class="data-value">{{ formatNumber(dataOverview.totalManuscripts) }}</div>
                  <div class="data-change"></div>
                </div>
                
                <div class="data-card">
                  <div class="data-title">点赞</div>
                  <div class="data-value">{{ formatNumber(dataOverview.totalLikes) }}</div>
                  <div class="data-change" v-if="dataOverview.likesIncrease > 0">
                    <el-icon><ArrowUp /></el-icon>
                    <span class="increase">+{{ dataOverview.likesIncrease }}</span>
                  </div>
                </div>
                
                <div class="data-card">
                  <div class="data-title">收藏</div>
                  <div class="data-value">{{ formatNumber(dataOverview.totalCollections) }}</div>
                  <div class="data-change" v-if="dataOverview.collectionsIncrease > 0">
                    <el-icon><ArrowUp /></el-icon>
                    <span class="increase">+{{ dataOverview.collectionsIncrease }}</span>
                  </div>
                </div>
                
                <div class="data-card">
                  <div class="data-title">硬币</div>
                  <div class="data-value">{{ formatNumber(dataOverview.totalCoins) }}</div>
                  <div class="data-change" v-if="dataOverview.coinsIncrease > 0">
                    <el-icon><ArrowUp /></el-icon>
                    <span class="increase">+{{ dataOverview.coinsIncrease }}</span>
                  </div>
                </div>
                
                <div class="data-card">
                  <div class="data-title">评论</div>
                  <div class="data-value">{{ formatNumber(dataOverview.totalComments) }}</div>
                  <div class="data-change" v-if="dataOverview.commentsIncrease > 0">
                    <el-icon><ArrowUp /></el-icon>
                    <span class="increase">+{{ dataOverview.commentsIncrease }}</span>
                  </div>
                </div>
                
                <div class="data-card">
                  <div class="data-title">弹幕</div>
                  <div class="data-value">{{ formatNumber(dataOverview.totalDanmaku) }}</div>
                  <div class="data-change" v-if="dataOverview.danmakuIncrease > 0">
                    <el-icon><ArrowUp /></el-icon>
                    <span class="increase">+{{ dataOverview.danmakuIncrease }}</span>
                  </div>
                </div>
                
                <div class="data-card">
                  <div class="data-title">分享</div>
                  <div class="data-value">{{ formatNumber(dataOverview.totalShares) }}</div>
                  <div class="data-change" v-if="dataOverview.sharesIncrease > 0">
                    <el-icon><ArrowUp /></el-icon>
                    <span class="increase">+{{ dataOverview.sharesIncrease }}</span>
                  </div>
                </div>
              </div>
              
              <!-- 核心数据趋势 -->
              <h4 class="chart-title">核心数据趋势</h4>
              <p class="chart-desc">近7天播放、点赞、评论的变化趋势</p>
              
              <!-- 稿件表现排行 -->
              <div class="manuscript-ranking-section">
                <div class="chart-header">
                  <div>
                    <h4 class="chart-title">稿件表现 TOP 5</h4>
                    <p class="chart-desc">播放量最高的5个稿件</p>
                  </div>
                  <el-button size="small" @click="downloadChart(manuscriptRankingChart, '稿件排行')">
                    <el-icon><Download /></el-icon> 下载图表
                  </el-button>
                </div>
                <div class="chart-container">
                  <div ref="manuscriptRankingChartRef" class="echarts-container ranking-chart"></div>
                </div>
              </div>
            </div>
            ...
            <div class="account-diagnosis" v-if="activeDataTab === 'diagnosis'">
              ...
              <div class="diagnosis-summary">
                ...
                <div class="summary-tip">
                  ...
                  <span>你的投稿活跃度为 {{ dataOverview.totalManuscripts }} 篇稿件，坚持创作，能帮助你快速进步与成长哦~</span>
                </div>
              </div>
              ...
              <div class="diagnosis-content">
                <div class="diagnosis-metrics">
                  <div class="metric-card">
                    ...
                    <div class="metric-info">
                      <div class="metric-label">稿件数</div>
                      <div class="metric-value">{{ dataOverview.totalManuscripts }}</div>
                    </div>
                  </div>
                  <div class="metric-card">
                    ...
                    <div class="metric-info">
                      ...
                      <div class="metric-value">{{ formatNumber(dataOverview.totalViews) }}</div>
                    </div>
                  </div>
                  <div class="metric-card">
                    <div class="metric-icon"><el-icon><StarFilled /></el-icon></div>
                    <div class="metric-info">
                      <div class="metric-label">总点赞</div>
                      <div class="metric-value">{{ formatNumber(dataOverview.totalLikes) }}</div>
                    </div>
                  </div>
                  <div class="metric-card">
                    ...
                    <div class="metric-info">
                      ...
                      <div class="metric-value">{{ formatNumber(dataOverview.totalFollowers) }}</div>
                    </div>
                  </div>
                </div>
                ...
                <div class="diagnosis-suggestions">
                  ...
                  <div class="suggestion-item" v-if="dataOverview.totalManuscripts < 10">
                    <el-icon class="suggestion-icon"><WarningFilled /></el-icon>
                    <span>稿件数量较少，建议保持稳定的更新频率</span>
                  </div>
                  <div class="suggestion-item" v-if="dataOverview.totalViews < 1000">
                    <el-icon class="suggestion-icon"><WarningFilled /></el-icon>
                    <span>播放量有待提升，可以尝试优化标题和封面</span>
                  </div>
                  <div class="suggestion-item" v-if="dataOverview.totalLikes < 100">
                    <el-icon class="suggestion-icon"><WarningFilled /></el-icon>
                    <span>互动率较低，可以加强与观众的互动</span>
                  </div>
                  <div class="suggestion-item" v-if="dataOverview.totalManuscripts >= 10 && dataOverview.totalViews >= 1000">
                    <el-icon class="suggestion-icon success"><CircleCheckFilled /></el-icon>
                    <span>表现不错，继续保持！</span>
                  </div>
                </div>
              </div>
            ...
            <div class="article-analysis" v-if="activeDataTab === 'article'">
              <div class="ranking-header">
                ...
                <div class="ranking-actions">
                  <el-select v-model="rankingSortBy" size="small" style="width: 120px;" @change="loadManuscriptRanking(rankingSortBy)">
                    <el-option label="播放量排行" value="views"></el-option>
                    <el-option label="点赞排行" value="likes"></el-option>
                    <el-option label="评论排行" value="comments"></el-option>
                  </el-select>
                </div>
              </div>
              
              <div class="ranking-list" v-if="manuscriptRanking.length > 0">
                <div class="ranking-item" v-for="(item, index) in manuscriptRanking" :key="item.id">
                  <div class="ranking-index" :class="{ 'top-3': index < 3 }">{{ index + 1 }}</div>
                  <div class="ranking-cover">
                    <img :src="item.coverUrl || getDefaultCover()" alt="封面" />
                  </div>
                  <div class="ranking-info">
                    <div class="ranking-title">{{ item.title }}</div>
                    <div class="ranking-meta">
                      <span class="meta-item"><el-icon><VideoPlay /></el-icon>{{ formatNumber(item.viewCount) }}</span>
                      <span class="meta-item"><el-icon><StarFilled /></el-icon>{{ formatNumber(item.likeCount) }}</span>
                      <span class="meta-item"><el-icon><ChatDotRound /></el-icon>{{ formatNumber(item.commentCount) }}</span>
                    </div>
                  </div>
                  <div class="ranking-stats">
                    <div class="stat-badge views">
                      <span class="stat-label">播放</span>
                      <span class="stat-value">{{ formatNumber(item.viewCount) }}</span>
                    </div>
                    <div class="stat-badge interaction">
                      <span class="stat-label">互动率</span>
                      <span class="stat-value">{{ item.interactionRate }}%</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="fans-analysis" v-if="activeDataTab === 'fans'">
              ...
              <div class="fans-overview">
                <div class="overview-header">
                  ...
                  <div class="overview-update">更新至{{ dataOverview.updateTime || '今日' }}</div>
                ...
                <div class="fans-stats-grid">
                  ...
                  <div class="stat-card highlight">
                    ...
                    <div class="stat-value">{{ formatNumber(dataOverview.totalFollowers) }}</div>
                    <div class="stat-icon">
                      <el-icon><UserFilled /></el-icon>
                    </div>
                  </div>
                  ...
                  <div class="stat-card">
                    ...
                    <div class="stat-value">{{ dataOverview.followersIncrease || 0 }}</div>
                  </div>
                  
                  <!-- 稿件数 -->
                  <div class="stat-card">
                    <div class="stat-title">稿件数</div>
                    <div class="stat-value">{{ dataOverview.totalManuscripts }}</div>
                  </div>
                  
                  <!-- 总播放量 -->
                  <div class="stat-card">
                    <div class="stat-title">总播放量</div>
                    <div class="stat-value">{{ formatNumber(dataOverview.totalViews) }}</div>
                  </div>
                  
                  <!-- 总点赞 -->
                  <div class="stat-card">
                    <div class="stat-title">总点赞</div>
                    <div class="stat-value">{{ formatNumber(dataOverview.totalLikes) }}</div>
                  </div>
                  
                  <!-- 总评论 -->
                  <div class="stat-card">
                    <div class="stat-title">总评论</div>
                    <div class="stat-value">{{ formatNumber(dataOverview.totalComments) }}</div>
                  </div>
                </div>
                ...
                <div class="fans-trend-chart">
                  <div class="chart-header">
                    <div>
                      <h4 class="chart-title">粉丝增长曲线</h4>
                      <p class="chart-desc">近{{ fansTimeRange.replace('d', '') }}天新增粉丝趋势</p>
                    </div>
                    <el-button size="small" @click="downloadChart(fansTrendChart, '粉丝增长曲线')">
                      <el-icon><Download /></el-icon> 下载图表
                    </el-button>
                  </div>
                  <div class="chart-container">
                    <div ref="fansTrendChartRef" class="echarts-container"></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- 互动管理 -->
            <div class="interaction-management" v-if="activeDataTab === 'interaction'">
              <div class="interaction-header">
                <h3 class="section-title">互动数据概览</h3>
              </div>
              <div class="interaction-stats-grid">
                <div class="stat-card">
                  <div class="stat-title">总评论</div>
                  <div class="stat-value">{{ formatNumber(dataOverview.totalComments) }}</div>
                </div>
                <div class="stat-card">
                  <div class="stat-title">总弹幕</div>
                  <div class="stat-value">{{ formatNumber(dataOverview.totalDanmaku) }}</div>
                </div>
                <div class="stat-card">
                  <div class="stat-title">总点赞</div>
                  <div class="stat-value">{{ formatNumber(dataOverview.totalLikes) }}</div>
