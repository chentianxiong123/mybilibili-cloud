-- 连麦功能数据库表
CREATE TABLE IF NOT EXISTS `live_linkmic` (
  `id` BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
  `room_id` BIGINT NOT NULL COMMENT '直播间ID',
  `streamer_id` BIGINT NOT NULL COMMENT '主播ID',
  `viewer_id` BIGINT NOT NULL COMMENT '观众ID（连麦者）',
  `viewer_name` VARCHAR(50) NOT NULL COMMENT '观众用户名',
  `status` TINYINT DEFAULT 0 COMMENT '状态: 0=申请中 1=已连接 2=已断开 3=被拒绝',
  `audio_enabled` TINYINT DEFAULT 0 COMMENT '音频是否开启: 0=关闭 1=开启',
  `video_enabled` TINYINT DEFAULT 0 COMMENT '视频是否开启: 0=关闭 1=开启',
  `max_linkmics` INT DEFAULT 3 COMMENT '最大连麦人数',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='直播连麦表';
