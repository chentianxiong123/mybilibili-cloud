...
- `ai` 视频处理状态通过 `video-process-analytics-topic` 发往 `analytics`，由 `video_process_events` 保存转码/音频/字幕/AI 摘要处理流水
- `interaction` / `comment` / `danmaku` 的点赞、投币、收藏、分享、评论、弹幕动作也会汇入 `manuscript_daily_metrics`
- `ai` 的视频全流程任务已落库到 `video_pipeline_tasks`，通过数据库租约认领执行；`PENDING` 可恢复，`RUNNING` 任务靠 `locked_until` 续租和过期失败兜住
...
- [ ] 视频转码流水线增强（当前 FFmpeg → HLS 三档、数据库租约认领和单线程执行已可用，但仍缺更细的分布式调度/优先级编排）
...
- `init/V14__video_pipeline_tasks.sql` (视频全流程任务持久化)
