...
Open source live streaming desktop app forked from Streamlabs Desktop.

The goal is to keep the Electron + OBS core and replace Streamlabs-specific services with the mybilibili live workflow.

This fork remains GPL-3.0 and keeps the original Streamlabs Desktop license.
...
We use pnpm as our package manager. Enable pnpm through Corepack:
...
The fork opens into `Live Control Room` by default. Sign in with a mybilibili
site account, load/create the live room, apply OBS settings, then use
`Edit Scenes` for the preserved OBS editor.

Backend expected in development:

- Gateway: `http://localhost:8080/api`
- Live room endpoints: `/live/room/my`, `/live/room/create`, `/live/room/{id}`
- RTMP server: `rtmp://{gateway-host}/live`

## Development Run Only

For source testing, do not package the app. Packaging pulls in signing,
installer, and distribution concerns that are not needed for the current
integration work.
...
During normal UI/code iteration, keep the backend services running separately
and restart the desktop app after `pnpm run compile`.
