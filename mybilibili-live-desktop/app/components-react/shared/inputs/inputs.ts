...
export function createBinding<TState extends object, TExtraProps extends object = {}>(
  ...
  const state = typeof stateGetter === 'function' ? (stateGetter as () => TState)() : stateGetter;
  const result = {} as any;
  for (const fieldName in state) {
    result[fieldName] = {
      ...
      onChange: (value: any) => {
        stateSetter({ [fieldName]: value } as Partial<TState>);
      },
    };
  }
  if (extraPropsGenerator) {
    for (const fieldName in state) {
      Object.assign(result[fieldName], extraPropsGenerator(fieldName));
    }
  }
  return result;
}
