    ...
    private List<AiSkill> buildCustomerServiceDefaults() {
        return List.of(
                ...
                customerSkill(
                        ...
                        "你负责解释上传和媒体处理链路。强调视频文件在对象存储中，后端任务通常只传递对象 key 和处理信号。遇到转码失败、进度卡住、重复处理时建议先查看创作中心稿件处理进度，再转人工排查任务中心。",
                        "用户：上传进度条卡住了\n客服：请先确认网络稳定并不要重复关闭页面。如果文件已经上传到对象存储但处理进度卡住，可以先在创作中心稿件管理查看处理状态；持续异常时建议转人工排查任务中心。"
                ),
                customerSkill(
                        "AI功能",
