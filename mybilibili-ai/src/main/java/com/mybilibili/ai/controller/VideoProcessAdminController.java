...
@RestController
@RequestMapping("/ai/admin/process")
public class VideoProcessAdminController {
    ...
    public Result<Map<String, Object>> getQueueInfo() {
        try {
            ...
            result.put("queueSize", waitingTranscode + waitingAudio + waitingSubtitle + waitingAi);
            result.put("message", "队列中有 " + (waitingTranscode + waitingAudio + waitingSubtitle + waitingAi) + " 个待处理任务");
        ...
        try {
            ...
            for (Video video : videos) {
                ...
                if (status == Video.PROCESS_STATUS_COMPLETED) {
                    processedCount++;
                } else if (status == Video.PROCESS_STATUS_TRANSCODING 
                    ...
                    processingCount++;
                } else if (status == Video.PROCESS_STATUS_TRANSCODE_FAILED
