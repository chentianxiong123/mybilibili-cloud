...
import type { Transform, EditingTemplatePrimitive } from "@mybilibili-studio/core";
import {
  ChromaKeyEngine,
...
const EmptyState: React.FC = () => (
  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center opacity-50">
    <p className="text-sm text-text-secondary mb-2">未选择素材</p>
      ...
      try {
        ...
        if (result.success) {
          if (result.errors.length > 0) {
            ...
            toast.success("SRT 导入完成", "字幕已添加到字幕轨道。");
          }
        } else {
