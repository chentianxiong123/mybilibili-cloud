  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-text-secondary">画幅比例</label>
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-text-secondary">质量</label>
          <Select value={value.quality} onValueChange={(v) => onChange({ ...value, quality: v as SeedreamInput["quality"] })}>
            <SelectTrigger className="h-8 text-sm"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="basic">基础（2K）</SelectItem>
              <SelectItem value="high">高清（4K）</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
