        ...
        <div className="flex flex-wrap gap-1.5">
          {PIPER_VOICES.map((voice) => (
            ...
            >
              ...
              <span className="text-[8px] opacity-70">{voice.gender === "female" ? "女" : "男"}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        {favoriteVoices.length > 0 && (
          <div className="space-y-1.5">
            <span className="text-[9px] text-text-muted flex items-center gap-1">
              <Star size={9} className="text-amber-400 fill-amber-400" /> 收藏音色
            </span>
            <div className="flex flex-wrap gap-1.5">
        ...
        {showAllVoices && (
          <div className="border border-border rounded-lg overflow-hidden">
            ...
            <div className="px-2 py-1 border-t border-border bg-background-secondary text-[8px] text-text-muted text-center">
              已显示 {filteredVoices.length} / {allVoices.length} 个音色
            </div>
          </div>
        )}
      </div>
