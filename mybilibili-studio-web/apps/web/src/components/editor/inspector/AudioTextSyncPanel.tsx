...
import { Button, LabeledSlider } from "@mybilibili-studio/ui";
import {
  getBeatSyncBridge,
  type BeatSyncState,
  DEFAULT_BEAT_SYNC_CONFIG,
} from "../../../bridges/audio-text-sync-bridge";
import type { SyncMode } from "@mybilibili-studio/core";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-text-secondary">
        <Music size={14} />
        <span className="text-[10px]">将片段同步到这段音频的节拍</span>
      </div>
      ...
      {!beatAnalysis ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-3 bg-primary/10 rounded-lg border border-primary/30">
            <div>
              <span className="text-[10px] text-text-secondary block">检测到 BPM</span>
          ...
          {clipsToSync.length > 0 && (
            <div className="p-2 bg-background-tertiary rounded-lg">
              <span className="text-[9px] text-text-muted">
                将把 {clipsToSync.length} 个片段同步到 {beatAnalysis.beats.length} 个节拍
              </span>
            </div>
          )}
            ...
            {showSettings && (
              <div className="space-y-3 p-3 bg-background-tertiary rounded-lg">
                <div>
                  ...
                  <div className="space-y-1">
                    {([
                      { value: "smart", label: "智能匹配", desc: "自动调整时长以贴合最近的节拍数量" },
                      { value: "one-per-beat", label: "一拍一段", desc: "每个片段正好占用一个节拍" },
                      { value: "preserve-duration", label: "保留时长", desc: "保留原始时长，只将起点吸附到节拍" },
                    ] as const).map((mode) => (
                      <button
                        key={mode.value}
